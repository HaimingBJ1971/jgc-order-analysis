...
from validators import scan_files, classify_files, validate_takeaway

class TakeawayView(QWidget):
    ...
    def __init__(self, app_state, parent=None):
        ...
        # View Title
        title_lbl = QLabel("平台外卖数据统计分析", self)
        title_lbl.setObjectName("view_title")
        main_layout.addWidget(title_lbl)
        
        # Drop Zone
        self.drop_zone = DropZone("拖拽 平台外卖 Excel 文件 到此处", self)
        ...
        param_layout.addWidget(QLabel("SQLite 数据库 (可选):", param_card), 0, 0)
        ...
        if not default_out:
            default_out = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "平台外卖统计", "output"))
        ...
        self.run_btn = QPushButton("▶️ 生成外卖统计分析", self)
    ...
    def handle_files_updated(self, paths):
        ...
        self.takeaway_files = classified.get("takeaway_excel", [])
    ...
    def run_validation(self):
        ...
        is_valid, messages = validate_takeaway(self.takeaway_files, self.db_path, output_dir)
    ...
    def trigger_run(self):
        ...
        spec = {
            "feature": "takeaway",
            "inputs": {
                "takeaway_files": self.takeaway_files,
